...
VideoSchema.statics.loadAnimation = function () {
    return this.find({"info.genre": "Animation"}).sort({ "info.english_name": 1 });
};
...
VideoSchema.statics.loadMarvel = function () {
    return this.find({"info.series": "Marvel"}).sort({ "info.english_name": 1 });
};

VideoSchema.statics.loadJapan = function () {
    return this.find({"info.country": "Japan"}).sort({ "info.english_name": 1 });
};

VideoSchema.statics.loadHK = function () {
    return this.find({"info.country": "Hong Kong"}).sort({ "info.english_name": 1 });
};

VideoSchema.statics.loadFrance = function () {
    return this.find({"info.country": "France"}).sort({ "info.english_name": 1 });
};

VideoSchema.statics.loadSF = function () {
    return this.find({"info.genre": "Sci-Fi"}).sort({ "info.english_name": 1 });
};
VideoSchema.statics.load90 = function () {
    return this.find({"info.year": {$in:[1990, 2000]}}).sort({ "info.english_name": 1 });
};
VideoSchema.statics.loadClassic = function () {
    return this.find({"info.year": {$lt:1990}}).sort({ "info.english_name": 1 });
};
VideoSchema.statics.loadWoodyAllen = function () {
    return this.find({"info.director": "Woody Allen"}).sort({ "info.year": -1 });
};
VideoSchema.statics.loadSeries = function () {
    return this.find({"info.series": {$ne: ""}}).sort({ "info.english_name": 1 });
};
VideoSchema.statics.loadMusic = function () {
    return this.find({$or:[{"info.genre":"Music"}, {"info.genre":"Musical"}]}).sort({ "info.english_name": 1 });
};
VideoSchema.statics.loadRomance = function () {
    return this.find({"info.genre": "Romance"}).sort({ "info.english_name": 1 });
};
